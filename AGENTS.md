...
前端不要依赖提示词文本，只依赖稳定 JSON Schema。当前 Harness 采用：
...
前端只消费最终 `VideoKnowledgePoint[]`；链路 3 复用审核后的
`source-knowledge-artifact.v1`，不读取候选卡或内部审核 Trace。

## 链路 3

链路 3 只接受 3–10 条已完成解析、主题相关的视频：
...
必须复用单视频 ASR、OCR、关键帧与链路 2 产物，不得重复解析视频。每个学习路径节点必须保留来源视频 ID 与时间范围；混合主题、证据不足或审核未通过时应拒绝发布。
...
> AI 一边把单条视频的知识主干组织成可跟随的问题和答案，一边在用户可能卡住的地方补上解释、求真和直观参照，并把多条同主题视频重构成可回溯的学习路径。
